...
from db_connector import connect_to_database, close_connection, get_next_available_row_id_from_db, \
    populate_config_table, setup_database
...
def get_user(user_id):
    ...
    if request.method == 'GET':
        ...
        logging.debug("Entered GET method call")
        ...
        try:
            ...
            try:
                logging.debug(f"retrieve user with user_id = %s ;", user_id)
                cursor.execute(f"SELECT * FROM {database}.users WHERE user_id = %s;", user_id)
                result = cursor.fetchone()

                if result:
                    logging.debug(f"retrieve user with user_id = %s has been retrieved;", user_id)
                    logging.debug(f"RESPONSE FROM QUERY  %s ", result[1])
                    # return jsonify(result)  # Return the result as JSON
                    return jsonify({"status": "ok", "user_name": result[1]}), 200

                else:
                    logging.debug(f"User with id %s does not exist;", user_id)
                    return jsonify({"status": "error", "reason": "no such user_id"}), 500

            except pymysql.MySQLError as query_error:
                logging.debug(f"Query execution error: {query_error}")

        finally:
            logging.debug("close all connections")
    ...
    if request.method == 'PUT':
        logging.debug("Entered PUT method call")
        ...
        try:
            ...
            cursor.execute(f"SELECT user_id FROM {database}.users WHERE user_id = %s", (user_id,))
            existing_user = cursor.fetchone()

            if not existing_user:
                logging.debug("Can't update user with id  %s that does not exist", user_id)
                return jsonify({"status": "error", "reason": "no such id."}), 500
            else:
                # Using parameterized query to prevent SQL injection
                update_query = f"UPDATE {database}.users SET name = %s WHERE user_id = %s"
                cursor.execute(update_query, (user_name, user_id))
                conn.commit()
                logging.debug("Update user ...with id %s", user_id)
            ...
            logging.debug(f"Query execution error: {exception}")
    ...
    if request.method == 'DELETE':
        logging.debug("Entered DELETE method call")
        conn = None
        cursor = None
        ...
        try:
            ...
            query = f"SELECT user_id FROM {database}.users WHERE user_id = %s"
            ...
            if not existing_user:
                ...
                delete_query = f"DELETE FROM {database}.users WHERE user_id = %s"
                cursor.execute(delete_query, user_id)
                conn.commit()
                logging.debug("Deleted entry from the database...with id %s", user_id)
...
def create_user():
    logging.debug("Entered POST method call")
    ...
    try:
        ...
        logging.debug(f"user name %s", user_name)
        # Connect to the database
        conn = connect_to_database()
        if not conn:
            return jsonify({"status": "error", "reason": "Database connection error."}), 500
        ...
        cursor.execute(f"SELECT user_id FROM {database}.users WHERE user_name = %s;", user_name)
        existing_user = cursor.fetchone()

        # Check if user_id already exists
        if existing_user:
            new_id_for_duplicate_name = get_next_available_row_id_from_db()
            logging.debug(f"new id after duplicate %s", new_id_for_duplicate_name)
        else:
            new_id_for_duplicate_name = 1

        insert_query = f"INSERT INTO {database}.users (user_id, user_name) VALUES (%s, %s)"
        cursor.execute(insert_query, (new_id_for_duplicate_name, user_name))
        conn.commit()
        logging.debug("Add new entry to the database...with id %s", new_id_for_duplicate_name)
        return jsonify(
