...
port = 3307  # You can set a default port here
...
def connect_to_database():
    try:
        ...
        if conn:
            logging.debug("Mysql connection succeeded")
            return conn
        else:
            # Log the connection information
            logging.debug(
                "MySQL connection failed using user: %s, password: %s, host: %s, database: %s",
                user, password, host, database
            )
    except pymysql.MySQLError as e:
        logging.error("ERROR connecting to the database: %s", e)
        logging.error(f"Mysql connection failed using user: %s, password: %s, host:%s, database: %",
                      'user', 'password', 'mysql-service', 'mydb')
        return None
...
def get_next_available_row_id_from_db():
    ...
    try:
        ...
        if not conn:
            logging.debug("Database connection error. Please check the code base ::: get_next_available_row_id_from_db")
        else:
            cursor = conn.cursor()

            try:
                # Execute the SQL query
                sql = f"SELECT MAX(user_id) FROM {database}.users"
                cursor.execute(sql)
...
def get_app_configuration_from_db():
    ...
    try:
        ...
        conn = connect_to_database()
        if not conn:
            logging.debug("Database connection error. Please check the code base ::: get_app_configuration_from_db")
        else:
            cursor = conn.cursor()
...
def setup_database():
    ...
    try:
        ...
        if not conn:
            logging.debug("Database connection error. Please check the code base::: setup_database")
        else:
...
def populate_config_table():
    ...
    try:
        ...
        if not conn:
            logging.debug("Database connection error. Please check the code base ::: populate_config_table")
        else:
            conn.autocommit(True)
            ...
            insert_data = {
                'id': '1',
                'url': 'http://127.0.0.1:55673/users/',
                'user_name': 'john',
                'browser': 'Chrome'
            }
            ...
            try:
                ...
                cursor.execute(select_query, ('http://127.0.0.1:55673/users/', 'john'))
